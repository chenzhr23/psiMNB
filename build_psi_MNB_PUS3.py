...
human_rna = pd.read_table('human_PUS_MNB_input_k-mer_PUS3.txt')
human_rna.head()
...
with open('PUS3_multinomialnb_model.pkl', 'wb') as f:
...
####preform prediction####
with open('PUS3_multinomialnb_model.pkl', 'rb') as f:
...
predicted_proba_df = pd.DataFrame(predicted_proba, columns=['non-PUS3', 'PUS3'])
to_pred = pd.concat([to_pred, predicted_proba_df], axis=1)
to_pred['predicted_PUS3_label'] = pd.DataFrame(predicted_label)
mapping = {0: 'non-PUS3', 1: 'PUS3'}
to_pred['enzyme'] = to_pred['predicted_PUS3_label'].map(mapping)
to_pred.head()
to_pred.to_excel("Day0_common_anno_group_redundance_mix_PUS3_prediction.xlsx",index=False)# pip install openpyxl