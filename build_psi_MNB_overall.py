...
with open('overall_multinomialnb_model.pkl', 'wb') as f:
...
####preform prediction####
with open('overall_multinomialnb_model.pkl', 'rb') as f:
...
predicted_proba_df = pd.DataFrame(predicted_proba, columns=['TRUB1', 'PUS3', 'PUS1'])
to_pred = pd.concat([to_pred, predicted_proba_df], axis=1)
to_pred['predicted_PUS_label'] = pd.DataFrame(predicted_label)
mapping = {0: 'TRUB1', 1: 'PUS3', 2:'PUS1'}
to_pred['enzyme'] = to_pred['predicted_PUS_label'].map(mapping)
to_pred.head()
to_pred.to_excel("Day0_common_anno_group_redundance_mix_overall_prediction.xlsx",index=False)# pip install openpyxl