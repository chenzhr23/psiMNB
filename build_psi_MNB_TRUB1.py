...
human_rna = pd.read_table('human_PUS_MNB_input_k-mer_TRUB1.txt')
human_rna.head()
...
# def center_kmers(structure, size=4):
#     center = math.ceil(len(structure)/2) - 1
#     kmers = []
#     for i in range(center-size+1, center+size):
#         kmers.append(structure[i:i+size])
#     return kmers
# human_rna['words'] = human_rna.apply(lambda x: center_kmers(x['Y_extendSeq_20nt']), axis=1)
...
with open('TRUB1_multinomialnb_model.pkl', 'wb') as f:
...
####preform prediction####
with open('TRUB1_multinomialnb_model.pkl', 'rb') as f:
    classifier = pickle.load(f)

to_pred = pd.read_table('Day0_common_anno_group_redundance_mix.txt')
to_pred.head()
# words = to_pred.apply(lambda x: center_kmers(x['Y_extendSeq_20nt']), axis=1)
...
predicted_proba_df = pd.DataFrame(predicted_proba, columns=['non-TRUB1', 'TRUB1'])
to_pred = pd.concat([to_pred, predicted_proba_df], axis=1)
to_pred['predicted_TRUB1_label'] = pd.DataFrame(predicted_label)
mapping = {0: 'non-TRUB1', 1: 'TRUB1'}
to_pred['enzyme'] = to_pred['predicted_TRUB1_label'].map(mapping)
to_pred.head()
to_pred.to_excel("Day0_common_anno_group_redundance_mix_TRUB1_prediction.xlsx",index=False)# pip install openpyxl