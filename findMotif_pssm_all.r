...
suppressMessages(library("optparse"))
if (!require('motifStack')) install.packages('motifStack');
...
option_list = list(
  make_option(c("-f", "--infile1"), type = "character", default = NULL,
              help = "input.xlsx.file [file]", metavar = "character"),
...
# positionSpecifc_motifs_sequences_append_info<-positionSpecifc_motifs_sequences_append_info %>% distinct(motifName, gene_biotype, .keep_all = TRUE) 
...
# positionSpecifc_motifs_sequences_append_info<-positionSpecifc_motifs_sequences_append_info[positionSpecifc_motifs_sequences_append_info$motifName=="motif1.similar1.motif",]
...
invisible(lapply(seq_along(motifName_list), function(x) {
  print(paste(paste("loopx ", x, ":", sep = ""), " processing ", paste("./homerResults/", str_replace_all(names(motifName_list)[x], "_[A-Z]*_[0-9]*pDist_[0-9]*", ""), sep = ""), sep = ""))
  #homerResults

  if (str_detect(motifName_list[[x]]$motifInSeq[1], "T")) {
    if (strsplit(motifName_list[[x]]$sequence[1], split = "")[[1]][11] == "T") {
      file <- paste("./homerResults/", str_replace_all(names(motifName_list)[x], "_[A-Z]*_[0-9]*pDist_[0-9]*", ""), sep = "")
      ...
      if (motifInSeq_pos > 11) {
        print(paste("motifInSeq position outrange 11 for ", paste("./homerResults/", str_replace_all(names(motifName_list)[x], "_[A-Z]*_[0-9]*pDist_[0-9]*", ""), sep = ""), sep = ""))
      } else {
        index <- 11 - motifInSeq_pos + 1
        if (is.na(motiffile$Y[index])) {
          print(paste("motifInSeq position U base get NA for ", paste("./homerResults/", str_replace_all(names(motifName_list)[x], "_[A-Z]*_[0-9]*pDist_[0-9]*", ""), sep = ""), sep = ""))
        } else {
          ...
          if (motiffile$Y[index] > 0.7) {
            ...
            # motif <- new("pcm", mat=as.matrix(pcm), name=str_replace_all(names(motifName_list)[x],"_[A-Z]*_[0-9]pDist_[0-9]",""))
            ...
            if (sum(pseudoU_anno_genetype_num$Freq) > 15) {
              ...
              pdf(paste(x, "_", outFile, "_", names(motifName_list)[x], "_", motifName_list[[x]]$motifSeq[1], "_all", ".pdf", sep = ""), width = 7, height = 7)
              print(pie_plot)
      ...
      print(paste("position 11 (21 nt) not U base for ", paste("./homerResults/", str_replace_all(names(motifName_list)[x], "_[A-Z]*_[0-9]pDist_[0-9]", ""), sep = ""), sep = ""))
    }
  } else {
    print(paste("motifInSeq not contain U base for ", paste("./homerResults/", str_replace_all(names(motifName_list)[x], "_[A-Z]*_[0-9]pDist_[0-9]", ""), sep = ""), sep = ""))
  }
